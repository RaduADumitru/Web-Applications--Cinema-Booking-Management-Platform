...
public interface ScreenSessionRepository extends JpaRepository<ScreenSession, Long>, JpaSpecificationExecutor<ScreenSession> {
    ...
    // Sessions on the given date whose movie is still active (not soft-deleted); used by the
    // reminder scheduler. The INNER join + deletedAt IS NULL skips sessions of a hidden movie so
    // the job neither sends reminders for a cancelled showing nor 500s on the lazy Movie proxy.
    // Unbounded by design (scheduler iterates one day's sessions).
    @Query("select s from ScreenSession s join s.movie m where s.date = :date and m.deletedAt is null")
    List<ScreenSession> findByDate(@Param("date") LocalDate date);
