...
6. **Actuator healthchecks.** The parent reactor pom already puts `spring-boot-starter-actuator` on every module, and each service exposes `health` via `management.endpoints.web.exposure.include`. Because the services set `server.servlet.context-path=/api/v1`, their actuator health is at `/api/v1/actuator/health`; the gateway (no context path) is at `/actuator/health`. Compose healthchecks `curl -f` these (the Dockerfile installs `curl`). Generous `start_period`/`retries` accommodate first-run in-container compilation. **Each service's `SecurityConfig` must permit `/actuator/**`** (`.requestMatchers("/actuator/**").permitAll()`) — otherwise the `anyRequest().authenticated()` rule returns `401` to the unauthenticated healthcheck `curl`, leaving the container stuck in `health: starting`. (The gateway has no Spring Security, so its `/actuator/health` is already open.) The `/actuator/**` paths are not registered in the gateway routes, so they remain reachable only over the Docker network and the debug host ports — not through the public gateway.

7. **CORS is terminated at the gateway.** Per-service Spring Security CORS does **not** reliably work through a reactive Spring Cloud Gateway (the gateway's WebFlux pipeline handles/forwards the preflight before the downstream CORS can apply), so the browser's credentialed login preflight fails with a CORS error. The fix (standard for an API gateway): a `CorsWebFilter` on the gateway (`gateway/config/CorsConfig.java`) allows the frontend origin (`security.cors.allowed-origins`, env `SECURITY_CORS_ALLOWED_ORIGINS`, default `http://localhost:4200`) with credentials + the `Authorization`/`Content-Type`/`X-XSRF-TOKEN` headers, and the three services **disable** their own Spring Security CORS (`http.cors(AbstractHttpConfigurer::disable)`) so the browser sees exactly one `Access-Control-Allow-Origin` (duplicate headers also break the browser). The frontend is unchanged (§9) — it still calls `http://localhost:8080/api/v1`; only the gateway emits CORS headers now.
